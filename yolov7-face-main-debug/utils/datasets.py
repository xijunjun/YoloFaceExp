...
def img2label_paths_custom(img_paths):
    # # Define label paths as a function of image paths
    # sa, sb = os.sep + 'images' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    # return ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
    return [imgpath_to_txtpath(x) for x in img_paths]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.label_files = img2label_paths_custom(self.img_files) 
        ...
        self.label_files = img2label_paths_custom(cache.keys())  # update
        if single_cls:
        ...
        if self.rect:
            ...
            if not tidl_load:
                ...
                self.batch_shapes = (np.array(shapes) * img_size / stride + pad).astype(np.int32) * stride
...
def augment_hsv(img, hgain=0.5, sgain=0.5, vgain=0.5):
    ...
    x = np.arange(0, 256, dtype=np.int3216)
...
def extract_boxes(path='../coco128/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            lb_file = Path(img2label_paths_custom([str(im_file)])[0])
...
def autosplit(path='../coco128', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        ...
        if not annotated_only or Path(img2label_paths_custom([str(img)])[0]).exists():  # check label
